...
function Sidebar({ setComponent }) {
  ...
  const handleLogout = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/users/logout`, { withCredentials: true });
    ...
    <>
      ...
      <div className={`w-64 h-full shadow-lg fixed top-0 left-0 bg-gray-50 z-40 transition-transform duration-300 transform sm:translate-x-0 ${show ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <ul className='space-y-6 mx-4'>
          ...
          <button onClick={gotoHome} className="w-full px-4 py-2 bg-red-500 rounded-lg hover:bg-rend-700 transition duration-300">HOME</button>
